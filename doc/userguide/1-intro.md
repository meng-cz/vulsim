...
SIMULATION() {
    for (tick_count = 0; tick_count < 20; ++tick_count) {
        sim_nextcycle(); // 推进一个完整时钟周期：执行事务交互并提交寄存器状态
    }
}
```
...
除此之外，Main 模块还可以声明顶层模块中定义的 `QUERY` 接口：

```cpp
QUERY(status, Status);
```

这类声明不会在 Main 中提供实现代码块，而是会在生成的仿真代码中被转发到顶层实例对应的 `QUERY` 成员函数上。因此它适合在单元测试或调试流程中读取顶层模块的当前稳定状态。
