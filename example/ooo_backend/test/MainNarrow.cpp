...
SERVICE_READY(mem_resp1, false, RESP(MemResponse) resp) {
    (void)resp;
}
...
SIMULATION() {
    ...
    for (uint64_t cyc = 0; cyc < max_cycles; ++cyc) {
        ...
        if (st.halted) {
            ...
            if (snap.x1 != 34ULL) {
                std::printf("narrow integration failed: x1=%llu expected=34\n", static_cast<unsigned long long>(snap.x1));
                std::exit(1);
            }
            if (mem[16] != 34ULL) {
                std::printf("narrow integration failed: mem[16]=%llu expected=34\n", static_cast<unsigned long long>(mem[16]));
                std::exit(1);
            }
            std::printf("ooo narrow passed: cycles=%llu committed=%llu x1=%llu mem16=%llu\n",
    ...
    std::printf("narrow integration failed: timeout\n");
    std::exit(1);
}
